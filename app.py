...
app = Flask(__name__)
CORS(app)  # allow all origins — your portfolio frontend can call this
...
print(f"[STARTUP] Model loaded. Input shape: {last_sequence.shape}")
print(f"[STARTUP] Features: {len(FEATURE_COLS)} | Targets: {TARGET_COLS}")


# ── Recursive prediction ──────────────────────────────────────
def predict_future(days=2):
...
def index():
    return jsonify({
        'service': 'VW Stock LSTM API',
        'status':  'running',
        'routes': {
            'GET  /health':       'health check',
            'POST /predict':      'get OHLC forecast',
            'GET  /last_prices':  'last known prices from model'
        }
    })
...
def predict():
    """
    Body: { "days": 2 }
    Returns forecast array with OHLC + confidence per day
    """
    try:
        body = request.get_json(force=True) or {}
        days = int(body.get('days', 2))
        days = max(1, min(days, 5))   # cap between 1 and 5

        preds = predict_future(days)

        CONF = ['HIGH', 'MEDIUM', 'LOW', 'POOR', 'POOR']

        # Generate business day dates from tomorrow
